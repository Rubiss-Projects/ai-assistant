...
for (const directory of [workspace, temporary, host, path.join(workspace, ".codex"), path.join(workspace, ".git")])
...
try {
    ...
    const child = spawn(process.env.CODEX_EXECUTABLE_PATH || "/usr/local/lib/codex/bin/codex", ["sandbox", "-C", workspace, "-P", "review",
        ...
        ...Object.entries(config.shell).map(([key, value]) => `${key}=${value}`), "/bin/sh", "-c", code], { env: { PATH: process.env.PATH, HOME: "/data", TMPDIR: temporary }, stdio: ["ignore", "pipe", "pipe"] });
    let output = "", errors = "";
