...
import { diagnosticUrl, isChallengePage } from "./browserDiagnostics.js";
...
export function extractWebpage(html, maxText = MAX_TEXT) {
    let title = "", text = "", structuredBytes = 0, structured = "";
    let truncated = false;
    const structuredData = [];
    const links = [];
    const images = [], embeddedPages = [];
    let baseHref;
    let titleSeen = false;
    const stack = [];
    ...
    const parser = new Parser({
        onopentag(name, attributes) {
            if (stack.length >= 512)
                throw new PublicFetchError("too_large", "The page exceeds the HTML nesting limit.");
            ...
            stack.push(current);
            if (name === "base" && !current.baseInert && baseHref === undefined && attributes.href !== undefined)
                baseHref = attributes.href;
            // Web components also expose article destinations through href attributes.
            if (!current.hidden && attributes.href && links.length < 120)
                links.push(attributes.href);
            if (!current.hidden && name === "iframe" && attributes.src && embeddedPages.length < 12)
                embeddedPages.push(attributes.src);
            if (!current.hidden && name === "img" && images.length < 24) {
                const source = attributes["data-src"] || attributes.src;
                if (source)
                    images.push(source);
            }
            if (!current.inert && name === "meta" && attributes.property === "og:image" && attributes.content && images.length < 24)
                images.push(attributes.content);
...
export const fetchWebpageResource = (url, options) => ebayListingUrl(url) ? fetchBrowserResource("ebay")(url, options) : fetchPublicResource(url, options);
export async function fetchWebpage(rawUrl, signal, fetchResource = fetchWebpageResource, options = {}) {
    ...
    try {
        ...
        if (!Number.isSafeInteger(offset) || offset < 0 || offset >= MAX_DOCUMENT_TEXT)
            throw new PublicFetchError("unsupported", "Text offset must be between 0 and 191999.");
        if (!Number.isSafeInteger(textLimit) || textLimit < 1 || textLimit > MAX_DOCUMENT_TEXT)
            throw new PublicFetchError("unsupported", "Invalid text chunk size.");
        let file;
        let reader = options.mode === "browser" ? "browser" : "http";
        ...
        if (["text/html", "application/xhtml+xml"].includes(resource.contentType)) {
            ...
            if (!feed)
                throw new PublicFetchError("unsupported", "The XML document is not an RSS or Atom feed.");
            ...
            throw new PublicFetchError("unsupported", "This URL did not return a webpage, text, JSON, RSS, or Atom. Use fetch_artifact for files.");
        if (isChallengePage(page.title, !page.structuredData.length && page.text.length < 1500 ? page.text : "")) {
            throw new PublicFetchError("challenge", "The source returned a verification/challenge page; its content is unavailable.", undefined, { ...resource.diagnostics, navigations: resource.diagnostics?.navigations ?? [], finalUrl: diagnosticUrl(resource.url), title: page.title, excerpt: page.text.slice(0, 2000) });
        }
        if (ebayListingUrl(url) && ebayListingUrl(resource.url) !== ebayListingUrl(url))
            throw new PublicFetchError("listing_mismatch", "The source returned a different page instead of the requested eBay listing.", undefined, resource.diagnostics);
        ...
        try {
            if (page.baseHref !== undefined)
                baseUrl = new URL(page.baseHref, resource.url).href;
        }
        catch { /* Invalid base URLs use the document URL. */ }
        const resolveLinks = (values) => [...new Set(values.flatMap(link => { try {
