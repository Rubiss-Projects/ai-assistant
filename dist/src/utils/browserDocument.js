...
function snapshotDocument(bounds) {
    ...
    const escaped = (text, attribute = false) => {
    ...
    const walk = (node, depth) => {
        if (exceeded)
            return;
        if (++nodes > bounds.maxNodes || depth > 256) {
            exceeded = true;
            return;
        }
        if (node.nodeType === Node.TEXT_NODE) {
            escaped(node.nodeValue ?? "");
            return;
        }
        ...
        if (["style", "noscript", "template", "svg"].includes(name))
            return;
        if (name === "script") {
            if (element.getAttribute("type")?.toLowerCase() !== "application/ld+json" || element.hasAttribute("hidden") || element.getAttribute("aria-hidden") === "true")
                return;
            ...
            for (let child = node.firstChild; child && !exceeded; child = child.nextSibling) {
                ...
                for (let start = 0; start < text.length && !exceeded; start += 4096)
                    append(text.slice(start, start + 4096).replace(/</g, "\\u003c"));
            }
            append("</script>");
            return;
        }
        ...
        if (element.shadowRoot)
            for (let child = element.shadowRoot.firstChild; child && !exceeded; child = child.nextSibling)
                walk(child, depth + 1);
...
export async function boundedBrowserDocument(cdp, maxBytes) {
