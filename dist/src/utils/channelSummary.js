...
export function isChannelSummaryRequest(prompt) {
...
function rangeFor(prompt, invocation) {
    ...
    const validate = (range, matched = "") => {
        ...
        if (temporalConstraints.some(pattern => pattern.test(remainder.replace(/\brecent\s+(?:messages?|conversation|chat)\b/gi, ""))))
            return "Unsupported range. " + guidance;
        return range;
    };
    const channels = [...prompt.matchAll(/<#(\d+)>/g)];
    if (channels.some(match => match[1] !== invocation.channelId))
        return "Request the summary in the channel you want summarized.";
    const previous = prompt.match(/\bsince my (?:last|previous) message\b(?:\s+at\s+\d{1,2}:\d{2}(?:\s*[ap]m)?)?/i);
    if (previous)
        return validate({ kind: "last" }, previous[0]);
    const link = prompt.match(/\b(?:since|after)\s+(https:\/\/(?:ptb\.|canary\.)?discord(?:app)?\.com\/channels\/(\d+)\/(\d+)\/(\d+))/i);
    if (link) {
        if (link[2] !== invocation.guildId || link[3] !== invocation.channelId)
            return "The starting message must belong to this channel.";
        if (BigInt(link[4]) >= BigInt(invocation.id))
            return "The starting message must be older than this request.";
    ...
    if (duration) {
        ...
        if (amount < 1 || amount > CHANNEL_SUMMARY_CAPABILITIES.limits.durationAmount)
            return `Use a duration between 1 and ${CHANNEL_SUMMARY_CAPABILITIES.limits.durationAmount} minutes, hours, or days.`;
    ...
    if (count && (Number(count[1]) < 1 || Number(count[1]) > SCAN_LIMIT))
        return `Choose between 1 and ${SCAN_LIMIT} messages.`;
    if (count)
        return validate({ kind: "recent", count: Number(count[1]) }, count[0]);
...
export async function channelSummaryContext(invocation, prompt, client, canIncludeAuthor) {
    ...
    if (!invocation.guildId || !isChannelSummaryRequest(prompt))
        return null;
    const notice = (text) => `[Channel summary unavailable: ${text} Explain this limitation; do not invent a summary from other context.]\n\n${prompt}`;
    const range = rangeFor(prompt, invocation);
    if (typeof range === "string")
        return notice(range);
    return retrieveChannelSummary(invocation, prompt, range, client, canIncludeAuthor);
}
async function retrieveChannelSummary(invocation, prompt, range, client, canIncludeAuthor, signal) {
    const notice = (text) => `[Channel summary unavailable: ${text} Explain this limitation; do not invent a summary from other context.]\n\n${prompt}`;
    ...
    try {
        ...
        if (!channel || channel.isDMBased() || !("guildId" in channel) || channel.guildId !== invocation.guildId || !("messages" in channel))
            return notice("This channel has no readable message history.");
        for (const id of [requester, client.user?.id]) {
            if (!id || !channel.permissionsFor(id)?.has([PermissionFlagsBits.ViewChannel, PermissionFlagsBits.ReadMessageHistory]))
                return notice("You and the bot both need View Channel and Read Message History permissions.");
            if (channel.isThread() && channel.type === ChannelType.PrivateThread && !channel.permissionsFor(id)?.has(PermissionFlagsBits.ManageThreads)) {
                if (!await channel.members.fetch(id).catch(() => null))
                    return notice("Private thread membership could not be verified.");
            }
        }
        if (range.kind === "after") {
            const startingMessage = await client.rest.get(Routes.channelMessage(channel.id, range.id)).catch(() => null);
            if (!startingMessage || startingMessage.id !== range.id || startingMessage.channel_id !== channel.id)
                return notice("The starting message could not be retrieved. Supply an existing message link or a recent-message count.");
        }
        ...
        while (scanned < SCAN_LIMIT && !done) {
            ...
            const page = await client.rest.get(Routes.channelMessages(channel.id), { query: new URLSearchParams({ before, limit: String(limit) }) });
            if (page.length === 0) {
                done = true;
                break;
            }
            ...
            if (!ordered.length)
                return notice("History pagination did not advance. Please try again.");
            for (const message of ordered) {
                scanned++;
                if (range.kind === "last" && message.author.id === requester) {
                    anchorFound = true;
                    anchor = `https://discord.com/channels/${invocation.guildId}/${channel.id}/${message.id}`;
                    done = true;
                    break;
                }
                if ((range.kind === "after" && BigInt(message.id) <= BigInt(range.id)) || (range.kind === "time" && Date.parse(message.timestamp) <= range.timestamp)) {
                    done = true;
                    break;
                }
            ...
            if (page.length < limit || (range.kind === "recent" && scanned >= range.count))
                done = true;
        }
        if (range.kind === "last" && !anchorFound)
            return notice(`Your previous message was not found within ${scanned} scanned messages. Supply a starting message link or a recent-message count.`);
        const records = [];
        let size = 0;
        let clipped = false;
        ...
        for (const message of selected) {
            ...
            if (size + record.length + 1 > TEXT_LIMIT) {
                clipped = true;
                break;
            }
            records.push(record);
...
export function channelHistoryResolver(invocation, client, canIncludeAuthor = () => true) {
    ...
    return async (args, signal) => {
        if (!source.guildId)
            throw new Error("Channel history is unavailable in DMs. Use the existing conversation context.");
        const fields = { previous_message: [], recent: ["count"], after_message: ["message_url"], relative_time: ["amount", "unit"] };
        const kind = typeof args.range === "string" ? args.range : "";
        if (!Object.hasOwn(fields, kind) || Object.keys(args).some(key => !["run_id", "range", ...fields[kind]].includes(key)))
            throw new Error("Choose one supported range without additional constraints.");
        const integer = (value, limit) => {
            if (typeof value !== "string" || !/^\d+$/.test(value) || !Number.isSafeInteger(Number(value)) || Number(value) < 1 || Number(value) > limit)
                throw new Error(`Use an integer between 1 and ${limit}.`);
            return Number(value);
        };
        let range;
        if (kind === "previous_message")
            range = { kind: "last" };
        else if (kind === "recent")
            range = { kind: "recent", count: args.count === undefined ? CHANNEL_SUMMARY_CAPABILITIES.limits.defaultMessages : integer(args.count, SCAN_LIMIT) };
        else if (kind === "relative_time") {
            const units = { minutes: 60_000, hours: 3_600_000, days: 86_400_000 };
            if (typeof args.unit !== "string" || !Object.hasOwn(units, args.unit))
                throw new Error("Choose minutes, hours, or days.");
            ...
            if (!link || link[1] !== source.guildId || link[2] !== source.channelId)
                throw new Error("Provide a message link from the invoking channel.");
            if (BigInt(link[3]) >= BigInt(source.id))
                throw new Error("The starting message must be older than this request.");
