...
import { GITHUB_CONTRIBUTION_TIMEOUT_MS, githubContributionTools } from "./common/githubContributionToolDefinitions.js";
const endpoint = process.env.AI_GITHUB_BRIDGE_URL;
const token = process.env.AI_GITHUB_BRIDGE_TOKEN;
if (!endpoint || !token)
    throw new Error("GitHub contribution MCP must be started by the bot.");
const server = new Server({ name: "ai-assistant-github-contributions", version: "1.0.0" }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({ tools: githubContributionTools(process.env.AI_GITHUB_CODEX_REVIEWS === "true") }));
