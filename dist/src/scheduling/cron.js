...
export function scheduleHasEnded(task, now = Date.now()) {
    return task.endAt !== undefined && now >= task.endAt;
}
export function scheduleHasStarted(task, now = Date.now()) {
    return task.startAt === undefined || now >= task.startAt;
}
export function parseStartAt(value, timezone) { return parseScheduleDate(value, timezone, "start"); }
export function parseEndAt(value, timezone) { return parseScheduleDate(value, timezone, "end"); }
/** Local date-times use the schedule timezone; an explicit offset fixes the instant. */
function parseScheduleDate(value, timezone, label) {
    ...
    const matches = (date) => [date.getFullYear(), date.getMonth() + 1, date.getDate(), date.getHours(), date.getMinutes(), date.getSeconds(), date.getMilliseconds()]
        .every((part, i) => part === components[i]);
    let date;
    try {
        // Reject normalized dates such as 24:00, even when an offset was supplied.
        if (!matches(new CronDate(local, "UTC")))
            throw new Error(invalid);
    ...
    if (!offset) {
        if (!matches(date))
            throw new Error(`That local ${label} time does not exist because of a clock change. Choose another time or include a UTC offset.`);
...
export function nextOccurrences(cron, timezone, from = Date.now(), count = 3, startAt) {
    ...
    try {
        ...
        throw new Error("Use an IANA timezone, such as America/New_York or UTC.");
    }
...
export function validateSchedule(cron, timezone, minimumMs, now = Date.now(), startAt) {
...
export function scheduleDescription(cron, timezone) {
