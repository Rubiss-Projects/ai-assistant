...
export class ScheduleStore {
    ...
    pendingRuns() {
        const rows = this.db.prepare("SELECT data FROM runs WHERE state IN ('queued','ready') ORDER BY started").all();
        return rows.map(row => JSON.parse(row.data));
    }
    ...
    recover(now) {
        ...
        OR (state='delivery_failed' AND json_extract(data, '$.error')=?)`).all(LEGACY_SAVED_OUTPUT);
