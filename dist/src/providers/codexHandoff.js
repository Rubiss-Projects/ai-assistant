...
export function codexHandoffOptions(options) {
...
export function parseHandoff(response) {
    ...
    if (value.summary.length > HANDOFF_LIMIT)
        throw new HandoffTooLongError();
    return value.summary.trim();
}
/** Allow modest overage; shorten larger summaries once within the same restricted run budget. */
export async function summarizeHandoff(thread, signal) {
    ...
    try {
        ...
        if (!(error instanceof HandoffTooLongError))
            throw error;
    }
...
export function withHandoff(prompt, summary) {
    return summary
