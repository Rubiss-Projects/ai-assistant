...
export class MemoryTurnJournal {
    ...
    put(record) {
        this.records.set(record.id, JSON.parse(JSON.stringify(record), (_k, v)=>v?.type === "Buffer" && Array.isArray(v.data) ? Buffer.from(v.data) : v));
    }
    all() {
        return [
            ...this.records.values()
...
export class FileTurnJournal {
    ...
    all() {
        return readdirSync(this.directory).filter((n)=>/^[a-f0-9]{64}\.json$/.test(n)).map((n)=>this.read(join(this.directory, n)));
    }
    close() {
        if (!this.closed) {
            this.closed = true;
...
export class ConversationService {
    ...
    stopped = false;
    pending = 0;
    constructor(journal = new MemoryTurnJournal(), maxPending = 100, timeoutMs = 3_600_000){
        ...
        for (const record of journal.all()){
            if ([
                ...
                this.save({
                    ...record,
                    state: 'interrupted',
                    error: 'Process stopped before completion; execution or delivery may have occurred. Submit a new request explicitly.'
                });
            }
        }
    }
    ...
    async submit(input, host) {
        ...
        if (stored && stored.state !== 'generated') return {
            id,
            completion: Promise.resolve(stored),
            cancel () {}
        };
        ...
        const execute = async (executionKey)=>{
            ...
            try {
                ...
                return this.save({
                    ...current,
                    state: uncertainDelivery ? 'interrupted' : controller.signal.aborted ? 'cancelled' : 'failed',
        ...
        const completion = Promise.resolve().then(async ()=>{
            ...
            let result;
            await host.coordinate(destination, async ()=>{
                result = await run();
            });
            ...
            return this.save({
                ...record,
                state: controller.signal.aborted ? 'cancelled' : 'interrupted',
                error: 'Destination or admission interrupted; check diagnostics before resubmitting.'
            });
        }).finally(()=>{
            this.pending--;
        ...
        const handle = {
            id,
            completion,
            cancel: ()=>controller.abort(new Error('Cancellation requested.'))
        };
...
export function historyRange(args, input, port, resource) {
    ...
    if (!fields[selected] || Object.keys(args).some((k)=>![
            'run_id',
            'range',
            'scope',
            ...fields[selected]
        ].includes(k))) throw new Error('Incompatible history constraints; ask for clarification.');
    const integer = (v, max)=>{
        const n = Number(v);
        if (!Number.isSafeInteger(n) || n < 1 || n > max) throw new Error('Invalid history range; ask for clarification.');
        return n;
    };
    switch(args.range ?? 'recent'){
        case 'recent':
            return {
                kind: 'recent',
                count: integer(args.count ?? 100, 1000)
            };
        ...
        case 'relative_time':
            {
                ...
                return {
                    kind: 'time',
                    timestamp: Date.parse(input.receivedAt) - integer(args.amount, 1000) * unit
                };
            }
        default:
            throw new Error('Unsupported history range; ask for clarification.');
    }
}
export async function retrieveHistory(port, input, resource, range, signal, limits = {
    messages: 100,
    characters: 60_000,
    pages: 10,
    scanned: 1000
    ...
    if (range.kind === 'after' || range.kind === 'previous') {
        const index = range.kind === 'after' ? messages.findIndex((m)=>m.position === range.position) : messages.length - 1 - [
            ...messages
        ].reverse().findIndex((m)=>m.authorId === input.actor.userId);
    ...
    for (const m of [
        ...
        ...messages.filter((m)=>m !== root).reverse()
    ]){
