...
import { REVIEW_RESULT_SCHEMA, REVIEW_TIMEOUT_MS, validateReviewResult } from "./codexReviewProtocol.js";
...
export function assertReviewLogin() {
    if (process.platform !== "linux")
        throw new Error("The review worker requires Linux sandboxing.");
    if (["OPENAI_API_KEY", "CODEX_API_KEY", "OPENAI_BASE_URL"].some(name => process.env[name]))
        throw new Error("Review worker refuses API credentials or endpoint overrides.");
    ...
    if (auth.auth_mode !== "chatgpt" || auth.OPENAI_API_KEY)
        throw new Error("Sign the review worker into Codex with ChatGPT first.");
    return directory;
}
export async function runCodexReview(input, signal) {
    ...
    for (const dir of [workspace, temporary, path.join(workspace, ".codex"), path.join(workspace, ".git")])
    ...
    try {
        ...
        for (const stream of [child.stdout, child.stderr])
            stream.on("data", (chunk) => { bytes += chunk.length; if (bytes > 4_000_000) {
                exceeded = true;
                kill();
            } });
        ...
        try {
            ...
            signal.throwIfAborted();
            if (code !== 0 || exceeded || !fs.existsSync(output) || fs.statSync(output).size > 100_000)
                throw new Error("Codex review failed, timed out, or returned excessive output. Check the worker login and subscription limits.");
