...
export async function reactWithParticipationEmoji(message, value) {
    if (!participationEmojis(message).some(emoji => emoji.value === value))
        throw new Error("Participation emoji is no longer available.");
...
export async function participationContext(candidates, botId, canIncludeAuthor, slashRequest) {
    ...
    const mayReadHistory = !("permissionsFor" in channel) || allowed.every(message => channel.permissionsFor(slashRequest?.authorId ?? message.author.id)?.has([PermissionFlagsBits.ViewChannel, PermissionFlagsBits.ReadMessageHistory]));
    if (mayReadHistory) {
        try {
            const history = await channel.messages.fetch({ before: latest.id, limit: 30 });
            for (const message of history.values()) {
                if (message.author.id === botId || (!message.author.bot && canIncludeAuthor(message.author.id)))
            ...
            for (const candidate of allowed.slice(-10)) {
                if (candidate.reference?.messageId && !messages.has(candidate.reference.messageId)) {
                    ...
                    if (reference && reference.channelId === latest.channelId &&
                        (reference.author.id === botId || (!reference.author.bot && canIncludeAuthor(reference.author.id))))
                        messages.set(reference.id, reference);
                }
            }
        }
    ...
    return ordered.reverse().map(message => {
        ...
        if (value.replyToId)
            value.replyToAuthorId = messages.get(value.replyToId)?.author.id;
        return value;
    }).reverse();
}
