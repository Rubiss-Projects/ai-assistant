...
export class ArtifactTools {
    ...
    call(name, args) {
        const operation = this.queue.catch(()=>{}).then(async ()=>{
            ...
            for (const key of schema.required)if (typeof args[key] !== "string" || !args[key].trim()) throw new Error(`Missing ${key}.`);
            ...
            if (name === "attach_file") return this.attach(String(args.path), args.filename);
            ...
            if (![
                ...
                "h264",
                "hevc"
            ].includes(String(args.codec))) throw new Error("Unsupported target codec.");
    ...
    async webpage(args) {
        ...
        const chunk = (result)=>{
            ...
            return {
                ...result,
                text: result.text.slice(offset, offset + 24_000),
                nextOffset,
        ...
        const result = await this.readWebpage(url, this.controller.signal, undefined, {
            mode,
            textLimit: 192_000
        });
        this.controller.signal.throwIfAborted();
    ...
    reportLookup(args) {
        ...
        if (![
            "verified",
            "unavailable"
        ].includes(String(args.status))) throw new Error("Use verified or unavailable.");
        const url = lookupUrl(String(args.url));
        ...
        this.options.onLookup({
            ...
            status: args.status,
            summary: page.status === "unavailable" ? page.message : summary,
            ...args.status === "unavailable" ? {
                errorCode: page.status === "unavailable" ? page.errorCode : "missing_data"
            } : {}
        });
        return {
            status: "recorded"
    ...
    async save(data, name, contentType) {
        ...
        return {
            artifact_id: path.basename(filePath),
            path: filePath,
    ...
    sourceRoot(file) {
        ...
        if (workspacePathIsAllowed(this.run.workingDirectory, resolved)) {
            ...
            return {
                root: this.run.workingDirectory,
                file: resolved
            };
        }
        const providerRoot = this.providerSourceRoot?.();
        if (providerRoot && workspacePathIsAllowed(providerRoot, resolved)) return {
            root: providerRoot,
            file: resolved
        };
    ...
    async attach(file, filename) {
        ...
        const normalizedExtension = this.normalizedExtensions.get(source.file) ?? (path.extname(source.file).toLowerCase() !== extension.toLowerCase() ? extension : undefined);
    ...
    async fetch(args) {
        ...
        if ((process.env.DISCORD_ATTACHMENT_MODE ?? "native").trim().toLowerCase() !== "native") {
            ...
            if (!head.subarray(0, 8).equals(Buffer.from("89504e470d0a1a0a", "hex")) && !(head[0] === 0xff && head[1] === 0xd8 && head[2] === 0xff) && !/^GIF8[79]a/.test(head.toString("ascii")) && !(head.subarray(0, 4).toString() === "RIFF" && head.subarray(8, 12).toString() === "WEBP")) {
