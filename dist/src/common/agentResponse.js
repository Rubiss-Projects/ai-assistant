...
function loadAttachment(workingDirectory, requestedPath, maxBytes) {
 ...
 */
export function prepareAgentResponse(content, workingDirectory) {
    const requestedPaths = [];
    const text = content.replace(ARTIFACT_MARKER, (_marker, requestedPath) => {
        requestedPaths.push(requestedPath);
        return "";
    }).replace(/\n{3,}/g, "\n\n").trim();
    const maxBytes = configuredMilliseconds("AI_OUTPUT_ATTACHMENT_MAX_BYTES", DEFAULT_MAX_ATTACHMENT_BYTES, 1);
    const maxAttachments = configuredMilliseconds("AI_OUTPUT_ATTACHMENT_MAX_COUNT", DEFAULT_MAX_ATTACHMENTS, 1);
    const attachments = [];
    const warnings = [];
    const seen = new Set();
