    ...
    begin(caller, repositoryName) {
        return this.serial(caller, async () => {
            ...
            if (!record) {
                ...
                if (owned.filter(item => !item.closed).length >= 5 || owned.filter(item => item.createdAt > Date.now() - 86_400_000).length >= 10 || this.records.length >= 1000)
                    throw new Error("Contribution limit reached. Close completed PRs and check their status before starting more.");
