...
export class CodexReviewWorker {
    ...
    constructor(directory, run) {
        ...
        for (const file of fs.readdirSync(directory).filter(name => name.endsWith(".json"))) {
            ...
            if (`${reviewId(job.id)}.json` !== file || !["queued", "running", "completed", "failed"].includes(job.state))
                throw new Error("Invalid worker state.");
            // Never silently spend a second review after an interrupted invocation.
            if (["queued", "running"].includes(job.state)) {
                job.state = "failed";
                job.error = "Review interrupted by worker restart; retry explicitly if budget remains.";
                this.save(job);
            }
            this.jobs.set(job.id, job);
        }
    }
    save(job) { writeReviewState(path.join(this.directory, `${job.id}.json`), job); }
    get(id) { return this.jobs.get(reviewId(id)); }
    submit(raw) {
        ...
        if (previous) {
            if (previous.digest !== digest)
                throw new Error("Review ID was already used for a different snapshot.");
            return previous;
        }
        ...
        this.active = Promise.resolve().then(async () => {
            ...
            this.save(job);
            try {
                job.result = validateReviewResult(await this.run(input, this.abort.signal), input);
                job.state = "completed";
            }
            catch {
                job.state = "failed";
                job.error = "Codex review failed or was interrupted. Check the worker login, subscription limits, and operator logs before retrying.";
            }
            this.save(job);
...
export async function serveReviews(worker, socket = REVIEW_SOCKET) {
    if (fs.existsSync(socket)) {
        if (!fs.lstatSync(socket).isSocket())
            throw new Error("Review socket path is not a socket.");
    ...
    const server = createServer(async (req, res) => {
        ...
        try {
            ...
            if (req.method === "GET" && req.url?.startsWith("/jobs/")) {
                ...
                res.writeHead(job ? 200 : 404).end(JSON.stringify(job ?? { error: "Unknown review." }));
                return;
            }
            ...
            res.writeHead(409).end('{"error":"Review request rejected; check job status and budget before retrying."}');
        }
    });
    server.requestTimeout = 15_000;
    server.headersTimeout = 10_000;
    server.maxConnections = 4;
    await new Promise((resolve, reject) => { server.once("error", reject); server.listen(socket, () => { fs.chmodSync(socket, 0o600); resolve(); }); });
    return server;
}
export class ReviewSocketClient {
    socket;
    constructor(socket = REVIEW_SOCKET) {
        this.socket = socket;
    }
    call(method, endpoint, signal, body) {
        return new Promise((resolve, reject) => {
            const req = request({ socketPath: this.socket, path: endpoint, method, signal, timeout: 15_000, headers: { "content-type": "application/json" } }, res => {
                const chunks = [];
                let bytes = 0;
                res.on("data", (chunk) => { bytes += chunk.length; if (bytes > 150_000)
                    req.destroy(new Error("Excessive review response."));
                else
                    chunks.push(chunk); });
                res.on("error", reject);
                res.on("end", () => {
                    ...
                    if (res.statusCode !== 200) {
                        reject(new Error("Review worker unavailable or busy."));
                        return;
                    }
                    try {
                        ...
                        reject(new Error("Invalid review worker response."));
                    }
                });
            });
    ...
    get(id, signal) { return this.call("GET", `/jobs/${reviewId(id)}`, signal); }
    async submit(input, signal) {
        const job = await this.call("POST", "/jobs", signal, input);
        if (!job)
            throw new Error("Review worker did not acknowledge the job.");
        return job;
    }
}
