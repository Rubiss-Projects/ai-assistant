...
async function replyLong(interaction, content) {
    const chunks = chunkForDiscord(content);
    if (interaction.deferred)
        await interaction.editReply(discordTextOptions(chunks[0]));
    else
        await interaction.reply({ ephemeral: true, ...discordTextOptions(chunks[0]) });
...
export async function handleRuleset(interaction, subject, access) {
    ...
    try {
        ...
        if (sub === "list" || sub === "get") {
            ...
            const lines = rulesets.map((ruleset) => `- \`${ruleset.name}\` ${ruleset.enabled ? "enabled" : "disabled"} priority ${ruleset.priority} updated ${ruleset.updatedAt}\n${ruleset.instructions}`);
