...
try {
  ...
  if (fs.existsSync(copilotPkg) && !fs.existsSync(defsDir)) {
    ...
    console.log(
      `@github/copilot/definitions/ missing - healing (v${version}).`
    );
    execSync(
  ...
  console.warn(
    "Could not heal @github/copilot/definitions/. Re-install without --install-links:\n" +
      "   npm install -g github:Rubiss-Projects/ai-assistant"
  );
}
