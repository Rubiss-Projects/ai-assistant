...
| `USER_INSTRUCTION_MODE` | `off`; accepts `off`, `admin_only`, `admin_and_self`, `unfiltered` | Enables per-Discord-user ruleset injection. The implemented management mode is `admin_only`: admins can use `/ruleset` or ask the active agent to call `ruleset_tools`; normal users cannot mutate rulesets. |
| `USER_INSTRUCTION_RULESETS_FILE` | `~/.config/ai-assistant/user-instructions.json` | JSON file storing user instruction rulesets. Back this up with the rest of the bot configuration. |
