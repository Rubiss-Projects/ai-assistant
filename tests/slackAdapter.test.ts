...
function setup(){
 const dir=mkdtempSync(join(tmpdir(),'slack-adapter-'));const service=new ConversationService();const prompts:string[]=[],posts:Record<string,string>[]=[];let reads=0,authorized=true;
 const api={call:async(method:string,args?:Record<string,string>)=>{
  ...
  if(method==='conversations.members')return {ok:true,members:authorized?['U','FRIEND','BOT']:['FRIEND','BOT']};
 ...
 return {adapter,prompts,posts,reads:()=>reads,revoke:()=>{authorized=false},close:async()=>{await service.shutdown();rmSync(dir,{recursive:true,force:true})}};
}
