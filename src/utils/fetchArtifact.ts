...
export async function fetchPublicResource(rawUrl: string, options: {
  signal?: AbortSignal; maxBytes: number; standardPortsOnly?: boolean; timeoutMs?: number;
  ...
  try {
    ...
    catch { throw new PublicFetchError("policy_blocked", "Provide a valid public HTTP(S) URL."); }
    for (let hop = 0; hop <= 5; hop++) {
      ...
      if (options.standardPortsOnly && url.port) throw new PublicFetchError("policy_blocked", "Webpages must use the standard HTTP(S) ports.");
      url.hash = "";
      ...
      const result = await new Promise<{ redirect?: string; file?: PublicResource }>((resolve, reject) => {
        const request = (url.protocol === "https:" ? https : http).get(url, {
          ...
          if ([301, 302, 303, 307, 308].includes(status)) {
            ...
            response.destroy();
            if (!redirect) reject(new PublicFetchError("network_error", "Redirect has no destination."));
          ...
          if (Number(response.headers["content-length"] ?? 0) > maxBytes) {
            response.destroy(); reject(new PublicFetchError("too_large", `Input exceeds the ${maxBytes}-byte limit.`)); return;
          }
          if (response.headers["content-encoding"] && response.headers["content-encoding"] !== "identity") {
            response.destroy(); reject(new PublicFetchError("unsupported", "The source ignored the requested identity encoding.")); return;
          }
          const chunks: Buffer[] = [];
          let bytes = 0;
      ...
      try { url = new URL(result.redirect!, url); }
      catch { throw new PublicFetchError("policy_blocked", "The source returned an invalid redirect URL."); }
    }
