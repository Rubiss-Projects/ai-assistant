...
async function recalledMemories(guildId: string, prompt: string, client: Client, userId: string): Promise<DiscordMemory[]> {
  ...
  for (const { memory } of ranked) {
    ...
    if (visible.length >= MEMORY_LIMIT) break;
  }
  return visible;
}
...
async function searchChannel(channel: TextBasedChannel, queryTerms: readonly string[]): Promise<Message[]> {
  ...
  while (remaining > 0) {
    ...
    for (const message of batch.values()) {
      if (!message.author.bot && score(message.content, queryTerms) > 0) matches.push(message);
    }
