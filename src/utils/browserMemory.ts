...
const decodeMount = (value: string) => value.replace(/\\([0-7]{3})/g, (_all, octal) => String.fromCharCode(parseInt(octal, 8)));

/** Resolve this process's controller mount and membership, including host cgroup namespaces. */
export function requireBrowserMemoryLimit(read: (file: string) => string = file => fs.readFileSync(file, "utf8")): void {
  ...
  try {
    ...
    for (const line of read("/proc/self/mountinfo").trim().split("\n")) {
      ...
      if (!membership?.directory.startsWith("/")) continue;
      ...
      let memory: bigint | undefined, swapOrCombined: bigint | undefined;
      const minimum = (current: bigint | undefined, file: string) => {
        let value: string;
        try { value = read(file).trim(); } catch { return current; }
      ...
      while (true) {
        ...
        if (!hierarchical) {
          try { hierarchical = read(path.posix.join(directory, "memory.use_hierarchy")).trim() === "1"; } catch { /* An unverified v1 ancestor is not an effective bound. */ }
        }
        if (hierarchical) {
      ...
      if (memory !== undefined && memory > 0n && memory <= MAX_MEMORY && swapOrCombined !== undefined
        && (v2 ? memory + swapOrCombined <= MAX_MEMORY : swapOrCombined > 0n && swapOrCombined <= MAX_MEMORY)) return;
    }
  } catch { throw fail(); }
  throw fail();
}
