import { ChannelType, PermissionFlagsBits, type Client, type GuildChannel, type ThreadChannel } from "discord.js";
import type { ArtifactCandidate, ArtifactMessage } from "../providers/types.js";

export function discordMessageLocation(value: string): { guild: string; channel: string; message: string } | undefined {
  try {
    const url = new URL(value);
    if (!/^(?:(?:ptb|canary)\.)?discord(?:app)?\.com$/i.test(url.hostname) || url.protocol !== "https:") return;
    const match = url.pathname.match(/^\/channels\/(\d+|@me)\/(\d+)\/(\d+)\/?$/);
    if (match) return { guild: match[1], channel: match[2], message: match[3] };
  } catch { /* Not a Discord message link. */ }
}

export function urlsInText(content: string): string[] {
...
export function artifactMessageResolver(
  client: Client,
  requester: string,
  canIncludeAuthor: (author: string) => boolean = () => true,
): (url: string) => Promise<ArtifactMessage> {
  return async (url) => {
    ...
    const candidates: ArtifactCandidate[] = [...message.attachments.values()].map((file) => ({
    ...
    for (const link of links) {
      if (!candidates.some((candidate) => candidate.url === link)) candidates.push({ url: link, sourceMessage });
    }
    return { candidates: candidates.slice(0, 20) };
  };
}

export async function readArtifactMessage(client: Client, requester: string, url: string,
  canIncludeAuthor: (author: string) => boolean = () => true) {
  const location = discordMessageLocation(url);
  if (!location) throw new Error("Invalid Discord message URL.");
  const channel = await client.channels.fetch(location.channel);
  if (!channel || !("messages" in channel)) throw new Error("Discord channel is unavailable.");
  if (channel.isDMBased()) {
    ...
    const guildChannel = channel as GuildChannel | ThreadChannel;
    if (guildChannel.guildId !== location.guild) throw new Error("Discord message server does not match its channel.");
  ...
  if (!canIncludeAuthor(message.author.id)) throw new Error("This message is excluded by the bot's context policy.");
  return message;
}
