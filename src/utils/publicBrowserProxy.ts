...
export function publicBrowserUrl(raw: string): URL {
  let url: URL;
  try { url = new URL(raw); } catch { throw new PublicFetchError("policy_blocked", "Use a public HTTP(S) URL."); }
  ...
  if (net.isIP(host) && !isPublicAddress(host)) throw new PublicFetchError("policy_blocked", "Private network addresses are unavailable.");
  return url;
}

/** Resolve once for the actual socket: Chromium never resolves destination hosts itself. */
export async function publicBrowserAddress(host: string) {
...
export async function createPublicBrowserProxy(signal: AbortSignal, maxBytes = 64 * 1024 * 1024) {
  ...
  const stop = () => { closed = true; for (const socket of sockets) socket.destroy(); server.close(); };
  const track = (socket: Duplex) => {
    ...
    socket.on("data", (data: Buffer) => {
      bytes += data.length;
      if (bytes > maxBytes) { lastError = new PublicFetchError("too_large", "Browser transfer budget exceeded."); stop(); }
    });
  };
  const allowed = (request: http.IncomingMessage) => {
    if (closed || request.headers["proxy-authorization"] !== authorization) return false;
    if (++connections > 128) { lastError = new PublicFetchError("too_large", "Browser connection budget exceeded."); stop(); return false; }
    return true;
  };
  ...
  server.on("connection", socket => { if (closed) socket.destroy(); else track(socket); });
  server.on("connect", (request, client, head) => {
    void (async () => {
      if (!allowed(request)) { client.end("HTTP/1.1 407 Proxy Authentication Required\r\nProxy-Authenticate: Basic realm=reader\r\nConnection: close\r\n\r\n"); return; }
      // CONNECT accepts an authority only, never a path, userinfo, or alternate port.
      if (!/^(?:\[[0-9a-fA-F:]+\]|[a-zA-Z0-9.-]+):443$/.test(request.url ?? "")) throw new PublicFetchError("policy_blocked", "Only HTTPS tunnels on port 443 are available.");
  ...
  server.on("request", (request, response) => {
    void (async () => {
      if (!allowed(request)) { response.writeHead(407, { "Proxy-Authenticate": "Basic realm=reader" }); response.end(); return; }
      if (!["GET", "HEAD"].includes(request.method ?? "")) throw new PublicFetchError("policy_blocked", "Only page reads are available.");
      const url = publicBrowserUrl(request.url ?? "");
      if (url.protocol !== "http:") throw new PublicFetchError("policy_blocked", "HTTPS requires a tunnel.");
      const address = await publicBrowserAddress(url.hostname);
      if (closed || response.destroyed) return;
      const headers = { ...request.headers, host: url.host };
      for (const key of ["proxy-authorization", "proxy-connection", "connection", "upgrade", "content-length", "transfer-encoding"]) delete headers[key as keyof typeof headers];
      ...
      upstream.end();
    })().catch(error => { failed(error); if (!response.headersSent) response.writeHead(502); response.end(); });
  });
  ...
  } catch (error) { signal.removeEventListener("abort", stop); stop(); throw error; }
}
