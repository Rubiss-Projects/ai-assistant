...
function isTextFile(contentType: string | null, name: string): boolean {
...
export async function downloadFileAttachments(
  ...
  for (const attachment of attachments) {
    ...
    try {
      ...
      if (!reader) throw new Error("Empty download response.");
      try {
        while (true) {
...
export interface PreparedAttachments {
  textContext: string;
  fileAttachments: Array<{ path: string; displayName?: string; kind: "image" | "file"; binary?: boolean }>;
}
