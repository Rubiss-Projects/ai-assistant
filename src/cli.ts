...
async function setup(): Promise<void> {
  ...
  const provider = await promptVar(
    rl,
    "AI provider (copilot | codex | opencode)",
    "PROVIDER",
    existing,
    false
  ) || "copilot";
  ...
  if (provider === "codex") {
    const openaiKey = await promptVar(
      rl,
      "OpenAI API Key (optional if Codex CLI is logged in)",
      "OPENAI_API_KEY",
      existing,
      false
    );
    const codexModel = await promptVar(
      rl,
      "Default Codex model (e.g. gpt-5.6-sol)",
      "CODEX_MODEL",
      existing,
      false
    );
    ...
    const opencodeModel = await promptVar(
      rl,
      "Default OpenCode model (provider/model, e.g. openrouter/...)",
      "OPENCODE_MODEL",
      existing,
      false
    );
