...
if (!endpoint || !token) throw new Error("Ruleset MCP must be started by the bot.");
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    ...
    if (!response.ok) throw new Error("Ruleset bridge is unavailable or this session has expired.");
