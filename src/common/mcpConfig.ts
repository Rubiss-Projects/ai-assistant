 ...
 *
 * The provider decides whether to inject these into a Live session (Copilot
 * does; Codex/OpenCode read MCP from their own config), but all providers use
 * this loader for the `/mcp list` status view.
 */
...
export class McpConfigLoader {
  private static readonly GLOBAL_PATH: string =
  ...
  static status(
    workingDir?: string
  ): { name: string; source: string; enabled: boolean }[] {
