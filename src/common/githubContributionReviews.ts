...
export interface ReviewThread {
  ...
  path: string;
  line: number | null;
  comments: { nodes: ReviewComment[]; pageInfo: { hasNextPage: boolean } };
  pullRequest: { number: number; repository: { databaseId: number }; headRefOid: string; state: string };
}
...
export function isPublisherComment(comment: ReviewComment, botLogin: string): boolean {
...
export function reviewThreadVersion(thread: ReviewThread): string {
...
export function reviewThreadSummary(thread: ReviewThread) {
