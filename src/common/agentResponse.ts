...
async function prepareAgentResponse(
  ...
  const processPaths = async (paths: string[], candidateLimit = maxAttachments): Promise<void> => {
  ...
  if (attachments.length === 0) {
    ...
    for (let index = 0; index < fallbackArtifacts.length; index++) {
      ...
      if (imported.marker) await processPaths([imported.marker], recoveryCandidateLimit);
      if (imported.warning) {
        processedCandidates += 1;
