...
function mediaCommand(program: string, args: string[], signal: AbortSignal): Promise<string> {
...
export async function transcodeVideo(data: Buffer, codec: "av1" | "h264" | "hevc", signal: AbortSignal): Promise<Buffer> {
  if (busy) throw new Error("Another video conversion is running. Retry after it finishes.");
  busy = true;
  ...
  try {
    ...
    const probe = JSON.parse(await mediaCommand("ffprobe", ["-v", "error", ...inputOptions,
      "-show_streams", "-show_format", "-of", "json", input], deadline)) as {
    ...
    const completed = JSON.parse(await mediaCommand("ffprobe", ["-v", "error", ...inputOptions,
      "-show_entries", "format=duration:stream=codec_name,codec_type", "-of", "json", output], deadline)) as {
      streams: Array<{ codec_type: string; codec_name: string }>; format: { duration: string };
    };
