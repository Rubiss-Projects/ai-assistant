import { ChatInputCommandInteraction, Message, ThreadAutoArchiveDuration, type ThreadChannel } from 'discord.js';
...
export async function handleChat(
  interaction: ChatInputCommandInteraction, sessions: SessionManager,
  ...
  let createdThread: ThreadChannel | undefined;
  ...
  try {
    ...
    const run = () => executeDiscordTurn(sessions, interaction, key, message, undefined, {
      ...
      onProgress: async ({ elapsedMs }) => { await durableReply!.edit(progressMessage(elapsedMs)); },
      ...
      const prepared = await prepareSlashAttachments(message, interaction.client, interaction.user.id,
        imageAttachment, interaction, canIncludeContextAuthor,
        internal => sessions.runEphemeral(destination, internal), participationAttachments(context));
    ...
    const failure = userVisibleErrorMessage(err) ?? runTimeoutMessage(err) ?? (isPathError
      ? `❌ Invalid workspace: ${(err as Error).message}` : '❌ Something went wrong talking to the AI. Please try again.');
    ...
    else await interaction.reply({ content: failure, ephemeral: true }).catch(() => {});
  }
}
