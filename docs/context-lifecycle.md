...
If summarization fails, the original mapping remains and the request fails instead of running with stale policy. Once the replacement thread starts, its ID and pending handoff are saved atomically. The handoff remains available for retries and process restarts until the first successful turn. A persistence failure is an error, not a successful refresh. Session files can temporarily contain conversation summaries and should retain their private file permissions. This uses the SDK's public thread API, not private transcript rewriting or App Server.
